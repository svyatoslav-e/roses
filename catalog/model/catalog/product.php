...
class ModelCatalogProduct extends Model {
	...
	public function getBestSellerProducts($limit) {
		...
		public function getBestSellerProductsId($limit, $category_id) {

			$product_id_data = array();
        

			$sql = "SELECT p2c.product_id, SUM(op.quantity) AS total  FROM " . DB_PREFIX . "product_to_category p2c LEFT JOIN " . DB_PREFIX . "order_product op ON (op.product_id = p2c.product_id) LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND `category_id`='".(int)$category_id."' ORDER BY total DESC LIMIT " . (int)$limit;
			...
			foreach ($query->rows as $result) {
				$product_id_data[] = $result['product_id'];
			}
